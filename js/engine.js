...
function main() {
    ...
    divRain.height = canvasHeight;
    divRain.width = canvasWidth;
    divRain.style.border = '1px solid #6D545D';
    divRain.id = 'rain';
...
function createRain() {
    for (let i = 1; i < nbDrop; i++) {
        initialX = (window.innerWidth - canvasWidth) / 2;
        finalX = initialX + canvasWidth;
        let dropLeft = randRange(initialX, finalX);
        ...
        div.id = 'drop' + i;
        div.style.left = dropLeft
        div.style.top = dropTop;
        document.getElementById('rain').appendChild(div);
